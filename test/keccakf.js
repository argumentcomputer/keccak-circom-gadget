...
describe("keccakfRound test", function () {
	...
	const cir = await wasm_tester(path.join(__dirname, "circuits", "keccakfRound0_test.circom"));
	...
	const cir = await wasm_tester(path.join(__dirname, "circuits", "keccakfRound20_test.circom"));
	await cir.loadConstraints();
	// console.log("n_constraints", cir.constraints.length);
...
describe("keccakf test", function () {
    ...
    let cir;
    before(async () => {
	cir = await wasm_tester(path.join(__dirname, "circuits", "keccakf_test.circom"));
	await cir.loadConstraints();
	console.log("n_constraints", cir.constraints.length);
    });
